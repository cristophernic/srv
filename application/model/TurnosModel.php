...
class TurnosModel
{
    ...
    public static function getTurno($id_turno)
    {
        ...
        $sql = "SELECT turnos.turno_id, turnos.turno_profesional, turnos.turno_fechain, turnos.turno_turno, users.user_nombre FROM turnos WHERE turno_id = :turno_id LIMIT 1 INNER JOIN users ON turnos.turno_profesional = users.user_id";
        $query = $database->prepare($sql);
